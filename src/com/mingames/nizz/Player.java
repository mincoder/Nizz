...
public class Player {
	private com.mingames.nizz.level.Level level;
	public float xo;
	public float yo;
	public float zo;
	public float x;
	...
	public Player(com.mingames.nizz.level.Level level) {
		this.level = level;
		resetPos();
	}
	...
	public void tick() {
		...
		if ((org.lwjgl.input.Keyboard.isKeyDown(205))
			...
			xa += 16.0F;
		if ((org.lwjgl.input.Keyboard.isKeyDown(Keyboard.KEY_SPACE))
				|| (org.lwjgl.input.Keyboard.isKeyDown(Keyboard.KEY_RCONTROL))) {
			//if (onGround) {
				yd -= 0.05F;
			//}
		}
		if ((org.lwjgl.input.Keyboard.isKeyDown(Keyboard.KEY_RSHIFT))
		...
		moveRelative(xa*100, ya*100, onGround ? 0.02F : 0.005F);
